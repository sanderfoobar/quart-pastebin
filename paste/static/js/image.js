...
$(document).ready(() => {
    ...
    $('.box').each(function () {
        ...
        $form.on('submit', function (e) {
            // disable auto-submit when navigated backwards
            let delta = ((new Date().getTime() / 1000) - now);
            if (delta <= 0.5) {
                e.preventDefault();
                return;
            }
            ...
            if (isAdvancedUpload) {
                ...
                // image(s) expiration
                let expiration = $('form#editor').find(".chooseExpiration").find("select option:selected").attr("value");
                let url = `${$form.attr('action')}?expiration=${expiration}`;
